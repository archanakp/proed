...
if($exam->is_scheduled==1) {	 
    ...
    if ($now < $schedule_from or $now > $schedule_to) {
        ...
        printf(__('This %1$s %2$s available between %3$s and %4$s.', 'watupro'), WATUPRO_QUIZ_WORD, $time_msg, date(get_option('date_format').' '.get_option('time_format'), $schedule_from), date(get_option('date_format').' '.get_option('time_format'), $schedule_to));
