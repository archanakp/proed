...
class WTPQuestion {
	...
	static function add($vars) {
		...
		$sql = $wpdb->prepare("INSERT INTO ".WATUPRO_QUESTIONS." (exam_id, question, answer_type, 
			...
			no_negative, max_allowed_points, limit_words, title) 
			VALUES(%d, %s, %s, %d, %s, %d, %d, %s, %d, %d, %d, %s, %s, %s, %s, %d, %s, %d, %d, %d, %d, 
				%f, %d, %d, %s, %d, %d, %s, %d, %s, %d, %f, %f, %d, %d, %f, %d, %d, %d, %d, %f, %d, %s)", 
			...
			$vars['file_upload_required'], $vars['no_negative'], $vars['max_allowed_points'], $vars['limit_words'], $vars['title']);
	...
	static function edit($vars, $id) {
      ...
      self :: prepare_vars($vars);
		...
		$wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_QUESTIONS."
			...
			max_allowed_points=%f, limit_words=%d, title=%s
			WHERE ID=%d", 
			...
			$vars['reduce_points_per_hint'], $vars['reduce_hint_points_to_zero'], $vars['accept_rating'], 
			$vars['file_upload_required'], $vars['no_negative'], $vars['max_allowed_points'], $vars['limit_words'], $vars['title'], $id));
    ...
    static function select_all($exam) {
    	...
    	$user_ID = get_current_user_id();
		   ...
		   if(!empty($advanced_settings['tags'])) {
			...
			ON tC.ID=tQ.cat_id
			WHERE tQ.exam_id IN ($q_exam_id) AND tQ.is_inactive=0 $difficulty_sql $unseen_sql $qcat_id_sql $tags_sql $q_id_sql
    ...
    function process($_watu, $qct, $question_content, $ques, $ansArr, $correct, $points) {
      ...
      $div_columns_class = ($ques->num_columns > 1 and empty($ques->compact_format)) ? 'watupro-' . $ques->num_columns.'-columns ' : '';  
		...
		if($ques->answer_type == 'gaps') {			
			...
			$current_text = "<div class='$wrap_columns_class show-question [[watupro-resolvedclass]]'><div class='show-question-content' style='width: 50%; background: #f3e8e8; padding: 20px; line-height: 16px;'>" . $question_number;
   		...
   		if($ques->answer_type=='checkbox' and $ques->allow_checkbox_groups and strstr($ques->question, '{{{group-')) {
    		...
    		$current_text = "<div class='$wrap_columns_class show-question $rtl_class [[watupro-resolvedclass]]".$compact_class."' style='width: 50%; background: #f3e8e8; padding: 20px; line-height: 16px;'><div class='show-question-content'>"
