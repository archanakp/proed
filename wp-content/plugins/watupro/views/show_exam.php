...
if($exam->full_time_limit > 0):?>
	    <div id="timeNag" <?php if(!empty($_POST['watupro_start_timer'])):?>style="display:none;"<?php endif;?>>
		    ...
		    printf(__('This %s must be completed in %s minutes.', 'watupro'), 'test', round($exam->time_limit, 1));
		    if(empty($button)):?> <a href="#" onclick="WatuPRO.InitializeTimer(<?php echo $exam->time_limit*60?>, <?php echo $exam->ID?>, 1);return false;"><?php printf(__('Click here to start the %s', 'watupro'), 'test')?></a><?php endif;?>		   
	    </div>
