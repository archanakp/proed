<?php
add_shortcode('ability_test_series', 'ability_test_series');
function ability_test_series()
{
	global $wpdb;
	$prefix = $wpdb->prefix;
	...
	<ul class="nav nav-tabs md-tabs" id="myTabMD" role="tablist">
		...
		if(!empty($exam_6))
		{
			...
			<li class="nav-item bg-success">
				<a class="nav-link" role="tab" aria-controls="home-md"
		...
		if(!empty($exam_7))
		{
			?>
			<li class="nav-item bg-danger">
				<a class="nav-link active" id="profile-tab-md" data-toggle="tab" role="tab" aria-controls="profile-md"
	...
	<div class="tab-content card pt-5" id="myTabContentMD">
		<div class="tab-pane fade" id="home-md" role="tabpanel" aria-labelledby="home-tab-md">
	  	...
	  	<div class="tab-pane fade show active red" id="profile-md" role="tabpanel" aria-labelledby="profile-tab-md">
	    	<?php
	    	$exam_6 = $wpdb->get_row("SELECT * FROM ".$prefix."watupro_taken_exams where user_id = ".$user_id." and exam_id = 6");
			if(!empty($exam_6))
			{
			...
			{
				echo "Please Complete Personality Assessment quiz";
			}
			?>
	  	</div>
