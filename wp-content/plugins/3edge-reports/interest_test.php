<?php
add_shortcode('interests_test_series', 'interests_test_series');
function interests_test_series()
{
	global $wpdb;
	$prefix = $wpdb->prefix;
	...
	<ul class="nav nav-tabs md-tabs" id="myTabMD" role="tablist">
		...
		if(!empty($exam_8))
		{
			?>
			<li class="nav-item bg-danger">
				<a class="nav-link active" id="contact-tab-md" data-toggle="tab"  role="tab" aria-controls="contact-md"
		...
		else
		{
			...
			<li class="nav-item grey_color">
				<a class="nav-link active" id="contact-tab-md" data-toggle="tab" href="#contact-md" role="tab" aria-controls="contact-md"
			  aria-selected="false">Interests</a>
			</li>
			<?php
		}
		...
		<li class="nav-item bg-danger">
			<a class="nav-link" aria-controls="logical-reasoning"
	...
	<div class="tab-content card pt-5" id="myTabContentMD">
		<div class="tab-pane fade" id="home-md" role="tabpanel" aria-labelledby="home-tab-md">
	  	...
	  	<div class="tab-pane fade show active red" id="contact-md" role="tabpanel" aria-labelledby="contact-tab-md">
	  		<?php
			$exam_7 = $wpdb->get_row("SELECT * FROM ".$prefix."watupro_taken_exams where user_id = ".$user_id." and exam_id = 7");
			if(!empty($exam_7))
			{
			...
			{
				echo "Please Complete Abilities Assessment quiz";
			}
			?>
	  	</div>
